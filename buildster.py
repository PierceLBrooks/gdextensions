...
class Context(Element):
  ...
  def check(self, node, parent):
    ...
    if not (tag in self.nodeTags):
      self.record(node, "Node Tag Error @ \""+str(tag)+"\"...")
      return False
    if (parent == None):
      if (tag in self.nodeParents):
        self.record(node, "Node Parent Error 1 @ \""+str(tag)+"\"...")
        return False
    else:
      if not (tag in self.nodeParents):
        self.record(node, "Node Parent Error 2 @ \""+str(tag)+"\"...")
        return False
      if not (self.any in self.nodeParents[tag]):
        if not (parent.tag in self.nodeParents[tag]):
          self.record(node, "Node Parent Error 3 (\""+str(parent.tag)+"\" | \""+str(self.nodeParents[tag])+"\") @ \""+str(tag)+"\"...")
          return False
    if (tag in self.nodeAttributes):
      ...
      for attribute in attributes:
        ...
        if not (attribute in node.attrib):
          if not (optional):
            self.record(node, "Node Attribute Error @ \""+str(tag)+"\"...")
            return False
    return True
...
def handle(context, node, tier, parents):
  ...
  if (context.check(node, parent)):
    ...
    if ((tag in context.conditionals) and ("id" in node.attrib)):
        ...
        if not (ensure(context.get(id)).strip() == check):
          ...
          for child in parent:
            if (child.tag == "exists"):
              ...
              break
          if not (exists == None):
            call = handle(context, exists, tier, parents[:(len(parents)-1)])
            if not (call[0]):
              return null
            else:
      ...
      if (parent.tag in context.conditionals):
        ...
        if (parent.tag == "if"):
          if (context.find(id)):
            context.log(node, str(id)+" does not exist in data!")
            return null
        elif (parent.tag == "if_check"):
          check = parent.attrib["check"]
          if (ensure(context.get(id)).strip() == check):
            context.log(node, str(id)+" does not match \""+check+"\" check!")
            return null
          ...
          if not (exists == None):
            ...
            if not (call[0]):
              ...
              if (os.path.exists(exists)):
                context.log(node, str(id)+" does not match \""+exists+"\" check!")
                return null
          else:
            return null
        else:
  ...
  if not (context.error == None):
    context.log(node, str(context.error))
    result = False
  if not (result):
